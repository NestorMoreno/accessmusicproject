...
(function (global) {
  ...
  function loadMissingPatch(url, path, filename) {
    ...
    request.onload = function() {
      ...
      if (num_missing == 0) {
        ...
        MIDIjs.message_callback("Sonando: " + url + ' ...');
      }
    }
    request.send();
  }
  ...
  function play_WebAudioAPI_with_script_loaded(url) {
    ...
    request.onload = function() {
      ...
      if (0 < num_missing) {
        ...
        source.connect(context.destination);      // connect the source to the context's destination (the speakers)
        start_time = context.currentTime;
        MIDIjs.message_callback("Sonando2: " + url + ' ...');
      }
    }
    request.send();
  }
  ...
  function lock() {
        var state = $('#instrument').prop('disabled');
        $('#instrument,#tempo,#notacion,#submit,#desgercarMidi,#descargarPdf,#descargarBraille').prop('disabled', !state);
  ...
  if (audioMethod == "WebAudioAPI") {
    ...
    audio_status = "metodoAudio: WebAudioAPI" +
